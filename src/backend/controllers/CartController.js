...
export const removeItemFromCartHandler = function (schema, request) {
  ...
  try {
    ...
    userCart = userCart.filter((item) => {
      // console.log("userCart item==>", typeOf(item.data._id) , typeOf(productId))
    return Number(item.data._id) !== Number(productId)
    }
    );
  ...
  try {
    ...
    if (action.type === "increment") {
      userCart.forEach((product) => {
        if (Number(product.data._id) === Number(productId)) {
          product.qty += 1;
