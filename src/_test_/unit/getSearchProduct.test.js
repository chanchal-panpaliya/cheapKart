import {getSearchProduct} from '../../my-web/pages/ProductList/utlities';
...
describe("testing get Search Product ", () => {
    //get Search Product
    it("should get Search Product", () => {  
        let getdatalist = InitialReducerState.map(item=>item.data)
        let search = "apple";
        let expectedState = getdatalist.filter((item)=>item.title.toLowerCase().includes(search.toLowerCase()))
        const state = getSearchProduct(getdatalist,search)
