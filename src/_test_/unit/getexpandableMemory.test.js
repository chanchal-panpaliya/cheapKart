import {getexpandableMemory} from '../../my-web/pages/ProductList/utlities';
...
describe("testing get expandable Memory ", () => {
    // get expandable Memory 512GB , 1 TB
    it("should  get expandable Memory", () => {  
        let getdatalist = InitialReducerState.filter(item=>item.categoryName==="Mobiles")
        let expectedState = getdatalist.filter((item)=>item.ExpandableMemory==="512GB")
        const state = getexpandableMemory(getdatalist,"Mobiles","512GB")
        expect(state).toEqual(expectedState); 
    })

    // get expandable Memory 1 TB
    it("should  get expandable Memory", () => {  
        let getdatalist = InitialReducerState.filter(item=>item.categoryName==="Mobiles")
        let expectedState = getdatalist.filter((item)=>item.ExpandableMemory==="1 TB")
        const state = getexpandableMemory(getdatalist,"Mobiles","1 TB")
        expect(state).toEqual(expectedState); 
    })
})