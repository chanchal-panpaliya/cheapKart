import {getExtraOff} from '../../my-web/pages/ProductList/utlities';
...
describe("testing get Extra Off ", () => {
    // get Extra Off
    it("should  get Extra Off", () => {  
        let getdatalist = InitialReducerState.filter(item=>item.data)
        let extraOff = "10 - 50"
        let pricefiltersplit = extraOff.split("-");
        const expectedState =  getdatalist.filter((item) =>{
                if( (Number(pricefiltersplit[0]) <= Number(item.extraOff)) && (Number(pricefiltersplit[1]) >= Number(item.extraOff))){
                    return item
                }
            })
        
        const state = getExtraOff(getdatalist,"10 - 50")
