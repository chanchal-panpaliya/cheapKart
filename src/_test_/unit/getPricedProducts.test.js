import {getPricedProducts} from '../../my-web/pages/ProductList/utlities';
...
describe("testing range slider price", () => {
    it("should set the range slider price", () => {  
        let getdatalist = InitialReducerState.map(item=>item.data)
        let price = 500;
        let expectedState = getdatalist.filter((item) => Number(item.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','')) <= Number(price)).sort((item1, item2) =>{
            let price1 = item1.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','');
            let price2 = item2.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','');
            return price2 - price1
        })

        const state = getPricedProducts(getdatalist,price)
