import {getRatingProduct} from '../../my-web/pages/ProductList/utlities';
...
describe("testing get rating product", () => {
    //rating product
    it("should get rating product", () => {  
        let getdatalist = InitialReducerState.map(item=>item.data)
        let rating = 5;
        let expectedState = getdatalist.filter((item) => Math.floor(item.Rating) === rating)
        const state = getRatingProduct(getdatalist,rating)
