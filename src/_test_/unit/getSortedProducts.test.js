import {getSortedProducts} from '../../my-web/pages/ProductList/utlities';
...
describe("testing Sort", () => {
    //SORT DATA - LOW_TO_HIGH
    it("should set the LOW_TO_HIGH", () => {  
        let getdatalist = InitialReducerState.map(item=>item.data)
        const expectedState = getdatalist.sort((a,b)=>{
            let price1 = a.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','');
            let price2 = b.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','');
            return price1 - price2
        })

        const state = getSortedProducts(getdatalist,"LOW_TO_HIGH")
        expect(state).toEqual(expectedState); 
    })
    //SORT DATA - HIGH_TO_LOW
    it("should set the HIGH_TO_LOW", () => {  
        let getdatalist = InitialReducerState.map(item=>item.data)
        const expectedState = getdatalist.sort((a,b)=>{
            let price1 = a.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','');
            let price2 = b.saleingprice.replace(/\,(\d\d)$/, ".$1").replace(',','');
        ...
        const state = getSortedProducts(getdatalist,"HIGH_TO_LOW")
        expect(state).toEqual(expectedState); 
    })
})