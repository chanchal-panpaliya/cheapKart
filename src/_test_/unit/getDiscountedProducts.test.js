import {getDiscountedProducts} from '../../my-web/pages/ProductList/utlities';
...
describe("testing get discount product", () => {
    it("should get discount product", () => {  
        let getdatalist = InitialReducerState.map(item=>item.data)
        let discount = 30;
        let expectedState = getdatalist.filter((item)=>item.percentOff>=discount)
        const state = getDiscountedProducts(getdatalist,discount)
